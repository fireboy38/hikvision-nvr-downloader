...
判断标准：
- 如果看到多个 .mp4/.dav 文件同时创建 → 分段下载
- 如果只有一个文件持续增长 → 可能绕过了1GB限制
- 如果最终文件 >1.1GB → 确认绕过了1GB限制
"""
...
# 监控配置
MONITOR_DURATION_MINUTES = 5  # 监控时长
CHECK_INTERVAL_SECONDS = 1    # 检查间隔

class FileMonitor:
    def __init__(self, download_path):
        self.download_path = download_path
        self.file_history = defaultdict(list)  # 记录每个文件的大小历史
        self.created_files = set()              # 记录创建的文件
        self.start_time = datetime.now()

    def get_files(self):
        """获取下载目录中的所有文件"""
        if not os.path.exists(self.download_path):
            print(f"⚠️  下载目录不存在: {self.download_path}")
            return []
                ...
                if timestamp.timestamp() - file_info['mtime'] < 60:
                    ...
                    if len(self.file_history[name]) % 10 == 0 and len(self.file_history[name]) > 0:
                        ...
                        if len(self.file_history[name]) >= 2:
                            ...
                            if time_diff > 0:
                                growth_rate = (size - prev_size) / time_diff / 1024  # KB/s

                        print(f"[{timestamp.strftime('%H:%M:%S')}] 📥 {name[:30]} "
    ...
    def analyze(self):
        ...
        # 2. 文件大小分析
        print("\n2. 文件大小统计:")
        large_files = [f for f in final_files if f['size'] > 100 * 1024 * 1024]  # >100MB
        ...
        if len(self.created_files) > 1:
            ...
            print(f"✅ 只有一个文件: {final_file['name']}")
            print(f"   大小: {size_mb:.2f} MB ({size_gb:.3f} GB)")
            print()

            if size_mb > 1100:  # >1.1GB
                print("🎉 结论: 文件大小 >1.1GB，竞业达成功绕过了1GB限制！")
            elif size_mb > 900 and size_mb < 1100:
                print("⚠️  文件大小约1GB，可能被V30接口截断")
                print("   建议: 需要更长的监控时间来确认")
            else:
                print("❓ 文件大小 <1GB，无法判断是否绕过了限制")
                print("   建议: 需要下载更长的录像（至少1小时）才能确认")

        else:
            print("❓ 监控期间未检测到明显的下载行为")
...
def main():
    ...
    if not os.path.exists(DOWNLOAD_PATH):
        print(f"⚠️  下载目录不存在: {DOWNLOAD_PATH}")
